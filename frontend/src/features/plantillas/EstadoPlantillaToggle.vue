...
function toggle() {
  ...
  ui.toast(nuevoEstado === 'publicado' ? `Plantilla "${props.plantilla.codigo}" publicada` : `Plantilla "${props.plantilla.codigo}" archivada`);
}
</script>
...
<template>
  <button
    @click="toggle"
    :title="publicado ? 'Archivar plantilla' : 'Publicar plantilla'"
