...
<template>
  <div v-if="bloqueado" class="p-8">
    <div class="bg-surface-card rounded-xl shadow-card p-12 flex flex-col items-center text-center max-w-lg mx-auto mt-12">
      ...
      <h1 class="text-xl font-bold text-heading mb-2">Mentorías grupales</h1>
  ...
  <div v-else class="p-8">
    <div class="bg-surface-card rounded-xl shadow-card p-6 mb-6 flex items-center gap-4">
      <div class="w-12 h-12 rounded-xl bg-brand-50 text-brand-600 flex items-center justify-center shrink-0">
        <FontAwesomeIcon :icon="faChalkboardUser" class="w-5 h-5" />
      </div>
      <div>
        <h1 class="text-2xl font-bold text-heading leading-tight">Mentorías</h1>
        <p class="text-sm text-muted">Sesiones grupales en vivo con un mentor real — más económicas que una consultoría 1 a 1.</p>
      </div>
    </div>
